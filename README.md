...
tests/                 pytest suite -- retrieval, guardrails, Critic scoring, ASR, and TTS regression tests (see below)
...
Orchestrator. **ASR and TTS are implemented; RAG-connected voice queries and voice approval are
not.** This is a deliberate partial implementation of a stretch goal, not a claim the whole thing
is done. Neither piece has an offline fallback (unlike embeddings) — local ASR/TTS need a model +
inference stack (`openai-whisper` + torch + ffmpeg) not justified for this scope, so both require
`OPENAI_API_KEY` with no degraded path.
...
`tests/test_asr.py` and `tests/test_tts.py` codify both as regression tests — network-dependent
cases skip (not fail) when `OPENAI_API_KEY` is unset, since there's no offline path to fall back to;
pure-logic tests (word-overlap math, summary text construction) always run.
