...
tests/                 pytest suite -- retrieval, guardrails, Critic scoring, ASR, TTS, and voice-query regression tests (see below)
...
Orchestrator. **ASR, TTS, and RAG-connected voice queries are implemented; voice approval/rejection
into the Orchestrator is not.** This is a deliberate partial implementation of a stretch goal, not
a claim the whole thing is done. None of these have an offline fallback (unlike embeddings) — local
ASR/TTS need a model + inference stack (`openai-whisper` + torch + ffmpeg) not justified for this
scope, so all three require `OPENAI_API_KEY` with no degraded path.
...
`tests/test_asr.py`, `tests/test_tts.py`, and `tests/test_voice_query.py` codify all three as
regression tests — network-dependent cases skip (not fail) when `OPENAI_API_KEY` is unset, since
there's no offline path to fall back to; pure-logic tests (word-overlap math, summary text
construction) always run.
